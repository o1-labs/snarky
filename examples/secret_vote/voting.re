...
module Vote = {
  ...
  let typ: Typ.t(t, Constant.t) =
    Typ.transport(
      Boolean.typ,
      ~there=Constant.to_bool,
      ~back=Constant.of_bool,
    );
};
...
module Vote_proof = {
  ...
  module Witness = {
    ...
    let handler =
        (
          {merkle_index, commitment_randomness, voting_key},
          Snarky.Request.With({request, respond}),
        ) => {
      switch (request) {
      | Merkle_index => respond(Provide(merkle_index))
      | Voting_key => respond(Provide(voting_key))
      | Commitment.Commitment_randomness =>
        respond(Provide(commitment_randomness))
      | _ => failwith("TODO")
      };
    };
  };
  ...
  let public_input = () =>
    Data_spec.[Merkle_tree.typ, Nullifier.typ, Vote.typ];

  let main = (root, nullifier, _vote: Vote.t, ()) => {
    let index = exists(Merkle_tree.Index.typ, ~request=() => Merkle_index);
    let commitment = Merkle_tree.lookup(root, index);
    let voting_key = exists(Voting_key.typ, ~request=() => Voting_key);
  ...
  let create = (tree, witness: Witness.t, vote) => {
    Proof_system.prove(
      proof_system,
      ~handlers=[
        Merkle_tree.Constant.handler(tree),
...
let example = () => {
  ...
  let my_commitment =
    Commitment.Constant.commit(witness.commitment_randomness);
  let tree =
    Merkle_tree.Constant.of_commitments([
      my_commitment,
      ...List.init(5, ~f=_ =>
           Commitment.Constant.commit(Randomness.Constant.create())
         ),
    ]);
