...
Module Var.
  ...
  Class Var (t : Type) :={var_index :> Index t; var_create :> Create t}.
...
Module Linear_combination.
  ...
  Definition of_constant {t field : Type} `{Create t} `{Of_field t field} 
  c := match c with
       | None => create tt
       | Some c => of_field c
       end.

  Definition zero {t} {field} {A} `{field.Field field A} `{Of_field t field} :=
  of_field field.zero.

  Module Class.
    Class Linear_combination (t field var : Type) :={linear_combination_create :
                                                  Create t;
                                                 linear_combination_of_var :
                                                  Of_var t var;
                                                 linear_combination_of_field :
                                                  Of_field t field;
                                                 linear_combination_add_term :
                                                  Add_term t field var}.
  End Class.
  Include Class.
End Linear_combination.

Module R1CS_constraint. 
  Class Create (t linear_combination : Type) :={create :
                                               linear_combination ->
                                               linear_combination ->
  ...
  Module Class.
    Class R1CS_constraint (t linear_combination : Type) :={r1cs_constraint_create :>
                                                        Create t
                                                          linear_combination;
                                                       r1cs_constraint_set_is_square :>
                                                        Set_is_square t}.
  End Class.
  Include Class.
End R1CS_constraint.
