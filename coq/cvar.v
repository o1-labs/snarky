...
Section Cvar.
  ...
  let
    fix go scale constant terms t :=
      ...
      | Add x1 x2 =>
          let (c, ts) := go scale constant terms x1 in go scale c terms x2
      end in
  let (c, ts) := go field.one field.zero [] t in (Some c, ts).
  ...
  Definition sub (t1 t2 : t F V) := add t1 (scale t2 neg_one).

  Definition linear_combination (terms : list (F * t F V)) :=
  fold_left
    (fun acc (term : F * t F V) => let (c, t) := term in add acc (scale t c))
    terms (constant field.zero).

  Definition sum (vs : list (t F V)) :=
  linear_combination (map (fun v => (field.one, v)) vs).
