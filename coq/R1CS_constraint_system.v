...
Section R1CS_constraint_system.
  ...
  Class Add_constraint_with_annotation_aux :=
    { add_constraint_with_annotation_aux : sys -> constraint -> string -> sys }.

  Definition add_constraint_with_annotation
     `{Add_constraint_with_annotation_aux} (x : t) (c : constraint) (s : string) :=
    {| primary_input_size := primary_input_size x
    ; auxiliary_input_size := auxiliary_input_size x
  ...
  Class Is_satisfied_aux := { is_satisfied_aux : sys -> list field -> list field -> bool }.

  Definition is_satisfied `{Is_satisfied_aux} (x : t) (primary secondary : list field) :=
...
Module Class.
  Class R1CS_constraint_system field constraint sys :=
  { system_create :> Create sys
  ; system_add_constraint :> Add_constraint_aux constraint sys
  ; system_add_constraint_with_annotation :> Add_constraint_with_annotation_aux constraint sys
  ; system_check :> Check_aux sys
  ; system_is_satisfied :> Is_satisfied_aux field sys
  ; system_digest_aux :> Digest_aux sys }.
End Class.
Include Class.
