...
Section T.
  ...
  Definition override_label x label_opt : basic_with_annotation V :=
    {| basic := basic x
    ; annot := (match label_opt with Some x => Some x | None => annot x end) |}.
...
Section Snarky.
  ...
  Definition basic_to_r1cs_constraint (basic : Basic.t (cvar.t field var)) : constraint.
    destruct LC, R1C.
    refine (let of_var := Linear_combination.of_var in
      match basic with
      | Boolean v =>
        let lc := of_var v in
  ...
  Definition add (stack : list string) (t : t (cvar.t field var))
    ...
    List.fold_left (fun system t =>
      let label := match annot t with
        | Some label => label
  ...
  Definition eval_basic (t : Basic.t (cvar.t field var)) get_value :=
    ...
    | Boolean v =>
      ...
      orb (field.equal x field.zero) (field.equal x field.one)
  ...
  Definition eval (t : t (cvar.t field var)) get_value :=
     List.forallb (fun t => eval_basic (basic t) get_value) t.
