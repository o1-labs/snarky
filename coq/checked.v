...
Section Instances.
  ...
  Global
Instance checked_return : (Return (t F V sys req S)) := {
 ret :=fun A x => Pure x}.

  Global
Instance checked_map : (Map (t F V sys req S)) := {
 map :=(fix map' S A B (t : checked F V sys req S A) :
          (A -> B) -> checked F V sys req S B :=
          match
            t in (checked _ _ _ _ s a)
            return ((a -> B) -> checked F V sys req s B)
          with
          | Pure x => fun f => Pure (f x)
          | With_label s t k =>
              fun f => With_label s t (fun b => map' _ _ _ (k b) f)
          | With_constraint_system c k =>
              fun f => With_constraint_system c (map' _ _ _ k f)
          | As_prover x k => fun f => As_prover x (map' _ _ _ k f)
          | Add_constraint c t1 =>
              fun f => Add_constraint c (map' _ _ _ t1 f)
          | With_state p and_then t_sub k =>
              fun f =>
              With_state p and_then t_sub (fun b => map' _ _ _ (k b) f)
          | With_handler h t k =>
              fun f => With_handler h t (fun b => map' _ _ _ (k b) f)
          | Clear_handler t k =>
              fun f => Clear_handler t (fun b => map' _ _ _ (k b) f)
          | Exists typ c k =>
              fun f => Exists typ c (fun v => map' _ _ _ (k v) f)
          | Next_auxiliary k =>
              fun f => Next_auxiliary (fun x => map' _ _ _ (k x) f)
          end) S}.

  Global
Instance checked_bind : (Bind (t F V sys req S)) := {
 bind :=(fix bind' S A B (t : checked F V sys req S A) :
           (A -> checked F V sys req S B) -> checked F V sys req S B :=
           match
             t in (checked _ _ _ _ s a)
             return
               ((a -> checked F V sys req s B) -> checked F V sys req s B)
           ...
           | Pure x => fun f => f x
           | With_label s t k =>
               fun f => With_label s t (fun b => bind' _ _ _ (k b) f)
           | With_constraint_system c k =>
               fun f => With_constraint_system c (bind' _ _ _ k f)
           | As_prover x k => fun f => As_prover x (bind' _ _ _ k f)
           | Add_constraint c t1 =>
               fun f => Add_constraint c (bind' _ _ _ t1 f)
           | With_state p and_then t_sub k =>
               fun f =>
               With_state p and_then t_sub (fun b => bind' _ _ _ (k b) f)
           | With_handler h t k =>
               fun f => With_handler h t (fun b => bind' _ _ _ (k b) f)
           | Clear_handler t k =>
               fun f => Clear_handler t (fun b => bind' _ _ _ (k b) f)
           | Exists typ c k =>
               fun f => Exists typ c (fun v => bind' _ _ _ (k v) f)
           | Next_auxiliary k =>
               fun f => Next_auxiliary (fun x => bind' _ _ _ (k x) f)
           end) S}.
...
Module Run.
  Section Run_util.
    ...
    Record run_state (F sys' : Type) (req : Type -> Type) (S : Type) :={num_inputs :
                                                                    nat;
                                                                    input :
                                                                    list F;
                                                                    next_auxiliary :
...
(*Definition run {F sys S A} `{field.Field F} (num_inputs : nat) (input : list F) (aux : list F) (system : option sys)
  ...
  let system := match system with
    ...
    Definition get_value {S : Type} (rstate : run_state F sys' req S) :
  var -> F :=
  let num_inputs := num_inputs rstate in
  let get_one :=
    fun i =>
    if Compare_dec.le_lt_dec i num_inputs
    then nth (i - 1) (input rstate) field.zero
    else nth (i - num_inputs - 1) (aux rstate) field.zero in
  cvar.eval get_one.

    Definition store_field_elt {S : Type} (rstate : run_state F sys' req S) 
  x : run_state F sys' req S * var :=
  ...
  (x : option (as_prover.t F var S A)) (s : option S) :
  option S * option A :=
  match x, s with
  | Some x, Some s =>
  ...
  Definition add_constraint {S : Type} (c : Constraint.t var)
  ...
  Definition store_or_alloc {Var} {Value} {S}
  (typ : Typ.t F var sys req Var Value) (value : option Value)
  (rstate : run_state F sys' req S) :=
  match value with
  | Some value =>
  ...
  Section Run_aux.
    ...
    Fixpoint run_aux {S A : Type} (rstate : run_state F sys' req S)
(t : t F var sys req S A) {struct t} : run_state F sys' req S * A + _ :=
  match
    t in (checked _ _ _ _ s a)
    return (run_state F sys' req s -> run_state F sys' req s * a + _)
  with
  ...
  | Add_constraint c t1 =>
      fun rstate =>
      let failed :=
        (eval_constraints rstate && constraint.eval c (get_value rstate))%bool
        in
      if failed
      then
       inr
         (concat "\n"
            ["Constraint unsatisfied:"%string; constraint.annotation c;
            constraint.stack_to_string (stack rstate)])
  ...
  | With_state p and_then t_sub k =>
      ...
      | inl (rstate, y) =>
          let p1 :=
            match state rstate return (forall T, (_ -> T) -> option T) with
            | Some s_sub => fun T and_then => Some (and_then s_sub)
            | None => fun _ _ => None
            end _ and_then in
          let rstate := set_state s rstate in
          let (s, _) := run_as_prover rstate p1 s1 in run_aux rstate (k y)
      | inr err => inr err
      end
  | With_handler h t k =>
      fun rstate =>
  ...
  | Clear_handler t k =>
      fun rstate =>
      let handler := handler rstate in
      let rstate := set_handler [] rstate in
      match run_aux rstate t with
      | inl (rstate, y) => run_aux (set_handler handler rstate) (k y)
      | inr err => inr err
      end
  | Next_auxiliary k =>
      fun rstate => run_aux rstate (k (next_auxiliary rstate))
  | Exists typ p k =>
      fun rstate =>
      match state rstate with
      | Some s =>
          let (s', value) :=
            provider.run p (get_value rstate) s (handler rstate) in
          match value with
          | Some value =>
              let (rstate, var) :=
                typ_monads.Store.run (Typ.store typ value) store_field_elt
                  rstate in
              match
                run_aux (set_state (Some tt) rstate) (Typ.check typ var)
              with
              | inl (rstate, _) =>
      ...
      | None =>
          let (rstate, var) :=
            typ_monads.Alloc.run (Typ.alloc typ) alloc_var rstate in
          match run_aux (set_state None rstate) (Typ.check typ var) with
          | inl (rstate, _) =>
              run_aux (set_state None rstate)
                (k {| Handle.var := var; Handle.value := None |})
          | inr err => inr err
          end
      end
  end rstate.
