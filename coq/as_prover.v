...
Instance as_prover_map  f v s: (Map (t f v s)) := {
 map :=fun A B t f tbl s => let (s', x) := t tbl s in (s', f x)}.

  Global Polymorphic
Instance as_prover_bind  f v s: (Bind (t f v s)) := {
 bind :=fun A B t f tbl s => let (s', x) := t tbl s in f x tbl s'}.
...
Module T.
  ...
  fun t tbl s => t tbl s.
    ...
    Definition get_state {S : Type} : t F V S S := fun _tbl s => (s, s).
  ...
  fun f _tbl s => (f s, tt).
    ...
    Definition read_var (v : V) : t F V S F := fun tbl s => (s, tbl v).

    Definition read {f v sys s var value : Type} {request : Type -> Type}
  (typ : Typ.t f v sys request var value) (var : var) : t f v s value :=
