...
module Make (Field : Snarky_intf.Field.Extended) : Intf with type field := Field.t = struct
  ...
  let to_constant_and_terms = to_constant_and_terms ~equal:Field.equal ~add:Field.add
      ~mul:Field.mul ~zero:Field.zero ~one:Field.one
