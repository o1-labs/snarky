(** Sets the [eval_constraints] state. If [true], [run_unchecked] and
    [prove] will check that the constraint system is satisfied while
    evaluating the [Checked.t]. The default value is [false].

    Note: This flag will not evaluate any constraints added by
    [with_constraint_system] (or the underlying
    [Types.Checked.With_constraint_system]). For these, you should
    modify your code to use the normal [run_and_check] function. *)
