...
module Run = struct
  ...
  struct
    ...
    (* TODO: Make this throw when the elt is too big *)
    let field_to_int x = int_of_bits (List.take (Field.Constant.unpack x) 62)
    ...
    let to_field t = Field.Constant.of_int (to_enum t)

    let of_field x = Option.value_exn (of_enum (field_to_int x))

    let assert_equal x y = Field.Assert.equal x y

    let typ : (var, t) Typ.t =
      let check =
        if M.max = 1 then fun x ->
          make_checked @@ fun () -> assert_ (Constraint.boolean x)
        else fun x ->
          make_checked
          @@ fun () -> Field.Assert.lte ~bit_length x (Field.of_int M.max)
      in
      {(Typ.transport Field.typ ~there:to_field ~back:of_field) with check}

    let var_to_bits : var -> Boolean.var list = Field.unpack ~length:bit_length

    let to_bits t = int_to_bits ~length:bit_length (to_enum t)

    let if_ b ~(then_ : var) ~(else_ : var) = Field.if_ b ~then_ ~else_

    let var t : var = Field.constant (to_field t)
