...
module Run = struct
  module Make (Impl : Snark_intf.Run_basic) = struct
    ...
    let two_to_the n =
      let rec go acc i =
        if i <= 0 then acc else go (Field.Constant.add acc acc) (i - 1)
      in
      go Field.Constant.one n
    ...
    let div_pow_2 n (`Two_to_the k) =
      ...
      { upper_bound=
          Bignum_bigint.(
            divided_of_bits.upper_bound / pow (of_int 2) (of_int k))
      ; lower_bound=
          Bignum_bigint.(
            divided_of_bits.lower_bound / pow (of_int 2) (of_int k))
      ; var= divided_of_bits.var
      ; bits= divided_of_bits.bits }
    ...
    let clamp_to_n_bits t n =
      ...
      with_label "Number.clamp_to_n_bits" (fun () ->
          let k = pow2 n in
          if Bignum_bigint.(t.upper_bound < k) then t
          else
            let bs = to_bits t in
            let bs' = List.take bs n in
            let g = Field.project bs' in
            let fits = Field.equal t.var g in
            let r =
              Field.if_ fits ~then_:g
                ~else_:(Field.constant Field.Constant.(sub (two_to_the n) one))
            in
            { upper_bound= Bignum_bigint.(k - one)
            ; lower_bound= t.lower_bound
            ; var= r
    ...
    *)
      with_label "Number.(<)" (fun () ->
          if x.upper_bound < y.lower_bound then Boolean.true_
          else if x.lower_bound >= y.upper_bound then Boolean.false_
          else
            let bit_length =
              Int.max
                (bigint_num_bits x.upper_bound)
                (bigint_num_bits y.upper_bound)
            in
            let {Field.less; _} = Field.compare ~bit_length x.var y.var in
            less )
    ...
    *)
    ...
    let constant x =
      ...
      { upper_bound= n
      ; lower_bound= n
      ; var= Field.constant x
      ; bits=
          Some
            (List.init (bigint_num_bits n) ~f:(fun i ->
                 Boolean.var_of_value (Bigint.test_bit tick_n i) )) }

    let one = constant Field.Constant.one

    let zero = constant Field.Constant.zero

    let of_pow_2 (`Two_to_the k) =
      constant (Field.Constant.of_int (Int.pow 2 k))

    let if_ b ~then_ ~else_ =
      let var = Field.if_ b ~then_:then_.var ~else_:else_.var in
      let open Bignum_bigint in
      { upper_bound= max then_.upper_bound else_.upper_bound
      ; lower_bound= min then_.lower_bound else_.lower_bound
      ; var
    ...
    let ( * ) x y =
      ...
      with_label "Number.(*)" (fun () ->
          let upper_bound = x.upper_bound * y.upper_bound in
          if upper_bound < Field.size then
            let var = Field.mul x.var y.var in
            { upper_bound
            ; lower_bound= x.lower_bound * y.lower_bound
            ; var
            ; bits= None }
