...
bool mnt4753_ate_G1_precomp::operator==(const mnt4753_ate_G1_precomp &other) const
{
...
std::ostream& operator<<(std::ostream &out, const mnt4753_ate_G1_precomp &prec_P)
{
...
std::istream& operator>>(std::istream &in, mnt4753_ate_G1_precomp &prec_P)
{
    in >> prec_P.PX;
...
bool mnt4753_ate_dbl_coeffs::operator==(const mnt4753_ate_dbl_coeffs &other) const
{
...
std::ostream& operator<<(std::ostream &out, const mnt4753_ate_dbl_coeffs &dc)
{
    out << dc.c_H << OUTPUT_SEPARATOR << dc.c_4C << OUTPUT_SEPARATOR << dc.c_J << OUTPUT_SEPARATOR << dc.c_L;
    return out;
}

std::istream& operator>>(std::istream &in, mnt4753_ate_dbl_coeffs &dc)
{
    in >> dc.c_H;
    consume_OUTPUT_SEPARATOR(in);
    in >> dc.c_4C;
...
bool mnt4753_ate_add_coeffs::operator==(const mnt4753_ate_add_coeffs &other) const
{
    return (this->c_L1 == other.c_L1 &&
            this->c_RZ == other.c_RZ);
}

std::ostream& operator<<(std::ostream &out, const mnt4753_ate_add_coeffs &ac)
{
    out << ac.c_L1 << OUTPUT_SEPARATOR << ac.c_RZ;
    return out;
}

std::istream& operator>>(std::istream &in, mnt4753_ate_add_coeffs &ac)
{
    in >> ac.c_L1;
    consume_OUTPUT_SEPARATOR(in);
    in >> ac.c_RZ;
...
bool mnt4753_ate_G2_precomp::operator==(const mnt4753_ate_G2_precomp &other) const
{
...
std::ostream& operator<<(std::ostream& out, const mnt4753_ate_G2_precomp &prec_Q)
{
    ...
    for (const mnt4753_ate_dbl_coeffs &dc : prec_Q.dbl_coeffs)
    {
        out << dc << OUTPUT_NEWLINE;
    }
    out << prec_Q.add_coeffs.size() << "\n";
    for (const mnt4753_ate_add_coeffs &ac : prec_Q.add_coeffs)
    {
...
std::istream& operator>>(std::istream& in, mnt4753_ate_G2_precomp &prec_Q)
{
    in >> prec_Q.QX;
    consume_OUTPUT_SEPARATOR(in);
    in >> prec_Q.QY;
...
mnt4753_Fq4 mnt4753_final_exponentiation_last_chunk(const mnt4753_Fq4 &elt, const mnt4753_Fq4 &elt_inv)
{
    enter_block("Call to mnt4753_final_exponentiation_last_chunk");
    ...
    leave_block("Call to mnt4753_final_exponentiation_last_chunk");

    return result;
}

mnt4753_Fq4 mnt4753_final_exponentiation_first_chunk(const mnt4753_Fq4 &elt, const mnt4753_Fq4 &elt_inv)
{
    enter_block("Call to mnt4753_final_exponentiation_first_chunk");
    ...
    leave_block("Call to mnt4753_final_exponentiation_first_chunk");
    return elt_q2_over_elt;
}

mnt4753_GT mnt4753_final_exponentiation(const mnt4753_Fq4 &elt)
{
    enter_block("Call to mnt4753_final_exponentiation");
    ...
    leave_block("Call to mnt4753_final_exponentiation");
...
{
    enter_block("Call to mnt4753_affine_ate_precompute_G1");
    ...
    leave_block("Call to mnt4753_affine_ate_precompute_G1");
    return result;
}

mnt4753_affine_ate_G2_precomputation mnt4753_affine_ate_precompute_G2(const mnt4753_G2& Q)
{
    enter_block("Call to mnt4753_affine_ate_precompute_G2");
    ...
    const bigint<mnt4753_Fr::num_limbs> &loop_count = mnt4753_ate_loop_count;
    ...
    /* TODO: maybe handle neg
       ...
       {
       mnt4753_ate_add_coeffs ac;
       mnt4753_affine_ate_dbl_coeffs c;
       c.old_RX = RX;
       c.old_RY = -RY;
       old_RX_2 = c.old_RY.squared();
       c.gamma = (old_RX_2 + old_RX_2 + old_RX_2 + mnt4753_coeff_a) * (c.old_RY + c.old_RY).inverse();
       c.gamma_twist = c.gamma * mnt4753_twist;
       c.gamma_X = c.gamma * c.old_RX;
       result.coeffs.push_back(c);
       }
    */
    ...
    leave_block("Call to mnt4753_affine_ate_precompute_G2");
    return result;
}
...
{
    enter_block("Call to mnt4753_affine_ate_miller_loop");

    mnt4753_Fq4 f = mnt4753_Fq4::one();
    ...
    for (long i = NAF.size() - 1; i >= 0; --i)
    {
        ...
        mnt4753_Fq4 g_RR_at_P = mnt4753_Fq4(prec_P.PY_twist_squared,
        ...
        if (NAF[i] != 0)
        {
            ...
            if (NAF[i] > 0)
            {
                g_RQ_at_P = mnt4753_Fq4(prec_P.PY_twist_squared,
...
struct extended_mnt4753_G2_projective {
    ...
    void print() const
    {
        printf("extended mnt4753_G2 projective X/Y/Z/T:\n");
        X.print();
        Y.print();
        Z.print();
        T.print();
...
void doubling_step_for_flipped_miller_loop(extended_mnt4753_G2_projective &current,
                                           mnt4753_ate_dbl_coeffs &dc)
{
    ...
    const mnt4753_Fq2 F = (B+B+B) + mnt4753_twist_coeff_a * A; // F = 3*B +  a  *A
    const mnt4753_Fq2 G = F.squared(); // G = F^2

    current.X = -(E+E+E+E) + G; // X3 = -4*E+G
    current.Y = -mnt4753_Fq("8")*D + F*(E+E-current.X); // Y3 = -8*D+F*(2*E-X3)
...
void mixed_addition_step_for_flipped_miller_loop(const mnt4753_Fq2 base_X, const mnt4753_Fq2 base_Y, const mnt4753_Fq2 base_Y_squared,
                                                 ...
                                                 mnt4753_ate_add_coeffs &ac)
{
    ...
    const mnt4753_Fq2 E = I + I + I + I; // E = 4*I
    const mnt4753_Fq2 J = H * E; // J = H * E
    const mnt4753_Fq2 V = X1 * E; // V = X1 * E
    const mnt4753_Fq2 L1 = D - (Y1 + Y1); // L1 = D - 2 * Y1
...
{
    enter_block("Call to mnt4753_ate_precompute_G1");
    ...
    leave_block("Call to mnt4753_ate_precompute_G1");
    return result;
}

mnt4753_ate_G2_precomp mnt4753_ate_precompute_G2(const mnt4753_G2& Q)
{
    enter_block("Call to mnt4753_ate_precompute_G2");
    ...
    leave_block("Call to mnt4753_ate_precompute_G2");
    return result;
}
...
{
    enter_block("Call to mnt4753_ate_miller_loop");

    mnt4753_Fq2 L1_coeff = mnt4753_Fq2(prec_P.PX, mnt4753_Fq::zero()) - prec_Q.QX_over_twist;
    ...
    for (long i = loop_count.max_bits() - 1; i >= 0; --i)
    {
        ...
        mnt4753_Fq4 g_RR_at_P = mnt4753_Fq4(- dc.c_4C - dc.c_J * prec_P.PX_twist + dc.c_L,
        ...
        if (bit)
        {
            ...
            mnt4753_Fq4 g_RQ_at_P = mnt4753_Fq4(ac.c_RZ * prec_P.PY_twist,
    ...
    {
    	mnt4753_ate_add_coeffs ac = prec_Q.add_coeffs[add_idx++];
    	mnt4753_Fq4 g_RnegR_at_P = mnt4753_Fq4(ac.c_RZ * prec_P.PY_twist,
                                         -(prec_Q.QY_over_twist * ac.c_RZ + L1_coeff * ac.c_L1));
    	f = (f * g_RnegR_at_P).inverse();
    }

    leave_block("Call to mnt4753_ate_miller_loop");
...
{
    enter_block("Call to mnt4753_ate_double_miller_loop");

    mnt4753_Fq2 L1_coeff1 = mnt4753_Fq2(prec_P1.PX, mnt4753_Fq::zero()) - prec_Q1.QX_over_twist;
    mnt4753_Fq2 L1_coeff2 = mnt4753_Fq2(prec_P2.PX, mnt4753_Fq::zero()) - prec_Q2.QX_over_twist;
    ...
    for (long i = loop_count.max_bits() - 1; i >= 0; --i)
    {
        ...
        mnt4753_Fq4 g_RR_at_P1 = mnt4753_Fq4(- dc1.c_4C - dc1.c_J * prec_P1.PX_twist + dc1.c_L,
                                       dc1.c_H * prec_P1.PY_twist);

        mnt4753_Fq4 g_RR_at_P2 = mnt4753_Fq4(- dc2.c_4C - dc2.c_J * prec_P2.PX_twist + dc2.c_L,
        ...
        if (bit)
        {
            ...
            mnt4753_Fq4 g_RQ_at_P1 = mnt4753_Fq4(ac1.c_RZ * prec_P1.PY_twist,
                                           -(prec_Q1.QY_over_twist * ac1.c_RZ + L1_coeff1 * ac1.c_L1));
            mnt4753_Fq4 g_RQ_at_P2 = mnt4753_Fq4(ac2.c_RZ * prec_P2.PY_twist,
    ...
    if (mnt4753_ate_is_loop_count_neg)
    {
        ...
        mnt4753_ate_add_coeffs ac2 = prec_Q2.add_coeffs[add_idx];
    	++add_idx;
    	mnt4753_Fq4 g_RnegR_at_P1 = mnt4753_Fq4(ac1.c_RZ * prec_P1.PY_twist,
                                          -(prec_Q1.QY_over_twist * ac1.c_RZ + L1_coeff1 * ac1.c_L1));
    	mnt4753_Fq4 g_RnegR_at_P2 = mnt4753_Fq4(ac2.c_RZ * prec_P2.PY_twist,
                                          -(prec_Q2.QY_over_twist * ac2.c_RZ + L1_coeff2 * ac2.c_L1));

    	f = (f * g_RnegR_at_P1 * g_RnegR_at_P2).inverse();
    }

    leave_block("Call to mnt4753_ate_double_miller_loop");

    return f;
}

mnt4753_Fq4 mnt4753_ate_pairing(const mnt4753_G1& P, const mnt4753_G2 &Q)
{
    enter_block("Call to mnt4753_ate_pairing");
    ...
    leave_block("Call to mnt4753_ate_pairing");
    return result;
}

mnt4753_GT mnt4753_ate_reduced_pairing(const mnt4753_G1 &P, const mnt4753_G2 &Q)
{
    enter_block("Call to mnt4753_ate_reduced_pairing");
    const mnt4753_Fq4 f = mnt4753_ate_pairing(P, Q);
    const mnt4753_GT result = mnt4753_final_exponentiation(f);
    leave_block("Call to mnt4753_ate_reduced_pairing");
    return result;
}
