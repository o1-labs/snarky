...
std::ostream& operator<<(std::ostream &out, const r1cs_bg_ppzksnark_proof<ppT> &proof)
{
    ...
    out << proof.delta_prime << OUTPUT_NEWLINE;
    out << proof.y_s << OUTPUT_NEWLINE;
    out << proof.z << OUTPUT_NEWLINE;
                                                      ...
                                                      const r1cs_bg_ppzksnark_auxiliary_input<ppT> &auxiliary_input)
{
    ...
    libff::G1<ppT> g1_A = pk.alpha_g1 + evaluation_At + (r * d) * pk.delta_g1;

    /* B = beta + sum_i(a_i*B_i(t)) + s*delta */
    libff::Fr<ppT> sd = s * d;
    libff::G1<ppT> g1_B = pk.beta_g1 + evaluation_Bt.h + sd * pk.delta_g1;
    libff::G2<ppT> g2_B = pk.beta_g2 + evaluation_Bt.g + sd * pk.delta_g2;

    /* C = sum_i(a_i*((beta*A_i(t) + alpha*B_i(t) + C_i(t)) + H(t)*Z(t))/delta) + A*s + r*b - r*s*delta */
    libff::G1<ppT> g1_C = d.inverse() * (evaluation_Ht + evaluation_Lt) + s *  g1_A + r * g1_B - (r * sd) * pk.delta_g1;

    libff::leave_block("Compute the proof");

    libff::leave_block("Call to r1cs_bg_ppzksnark_prover");

    r1cs_bg_ppzksnark_proof<ppT> proof = r1cs_bg_ppzksnark_proof<ppT>(
        std::move(g1_A), std::move(g2_B), std::move(g1_C), d * pk.delta_g2,
        // return dummy values for z and y_s
        libff::G1<ppT>::zero(), libff::G1<ppT>::zero());
    proof.print_size();
