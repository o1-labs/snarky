...
void camlsnark_bn128_var_delete(variable<FieldT>* v) {
  std::cout << "camlsnark_bn128_var_delete\n" << v;
  delete v;
}
...
bool camlsnark_bn128_r1cs_constraint_system_equal(
...
std::string* camlsnark_bn128_proving_key_to_string(r1cs_gg_ppzksnark_proving_key<ppT>* pk) {
  std::stringstream stream;
  stream << *pk;
  return new std::string(stream.str());
}

r1cs_gg_ppzksnark_proving_key<ppT>* camlsnark_bn128_proving_key_of_string(std::string* s) {
...
void camlsnark_bn128_proving_key_delete(r1cs_gg_ppzksnark_proving_key<ppT>* pk) {
  delete pk;
}

bool camlsnark_bn128_proving_key_equal(
...
bool camlsnark_bn128_verification_key_equal(
  r1cs_gg_ppzksnark_verification_key<ppT>* vk1,
  r1cs_gg_ppzksnark_verification_key<ppT>* vk2) {
  return *vk1 == *vk2;
}

void camlsnark_bn128_verification_key_delete(r1cs_gg_ppzksnark_verification_key<ppT>* vk) {
  delete vk;
}
...
std::string* camlsnark_bn128_verification_key_to_string(r1cs_gg_ppzksnark_verification_key<ppT>* vk) {
  std::stringstream stream;
  stream << *vk;
  return new std::string(stream.str());
}

r1cs_gg_ppzksnark_verification_key<ppT>* camlsnark_bn128_verification_key_of_string(std::string* s) {
...
r1cs_gg_ppzksnark_verification_key<ppT>* camlsnark_bn128_keypair_vk(r1cs_gg_ppzksnark_keypair<ppT>* keypair) {
  return new r1cs_gg_ppzksnark_verification_key<ppT>(keypair->vk);
}

void camlsnark_bn128_keypair_delete(r1cs_gg_ppzksnark_keypair<ppT>* keypair) {
  delete keypair;
}

r1cs_gg_ppzksnark_keypair<ppT>* camlsnark_bn128_keypair_create(
    r1cs_constraint_system<FieldT>* sys) {
...
r1cs_gg_ppzksnark_proof<ppT>* camlsnark_bn128_proof_of_string(std::string* s) {
...
bool camlsnark_bn128_gm_proving_key_equal(
...
bool camlsnark_bn128_gm_verification_key_equal(
...
r1cs_constraint_system<FieldT>* camlsnark_bn128_bg_proving_key_r1cs_constraint_system(
...
std::vector<libff::G1<ppT>>*
camlsnark_bn128_bg_verification_key_query(r1cs_bg_ppzksnark_verification_key<ppT>* vk) {
  std::vector<libff::G1<ppT>>* res = new std::vector<libff::G1<ppT>>();
  res->emplace_back(vk->ABC_g1.first);
  for (size_t i = 0; i < vk->ABC_g1.rest.values.size(); ++i)
  {
...
libff::G2<ppT>*
camlsnark_bn128_bg_verification_key_delta(r1cs_bg_ppzksnark_verification_key<ppT>* vk) {
  return new libff::G2<ppT>(vk->delta_g2);
}

libff::Fqk<ppT>*
camlsnark_bn128_bg_verification_key_alpha_beta(r1cs_bg_ppzksnark_verification_key<ppT>* vk) {
  return new libff::Fqk<ppT>(vk->alpha_g1_beta_g2);
}
...
libff::G1<ppT>* camlsnark_bn128_bg_proof_a(r1cs_bg_ppzksnark_proof<ppT>* proof) {
  return new libff::G1<ppT>(proof->g_A);
}

libff::G2<ppT>* camlsnark_bn128_bg_proof_b(r1cs_bg_ppzksnark_proof<ppT>* proof) {
  return new libff::G2<ppT>(proof->g_B);
}

libff::G1<ppT>* camlsnark_bn128_bg_proof_c(r1cs_bg_ppzksnark_proof<ppT>* proof) {
  return new libff::G1<ppT>(proof->g_C);
}
