...
#ifndef LIBIOP_RELATIONS_R1CS_HPP_
#define LIBIOP_RELATIONS_R1CS_HPP_
...
public:
    ...
    linear_combination<FieldT> a_, b_, c_;
 ...
 * R1CS variable assignment for which each R1CS constraint is satisfied.
 *
 * NOTE:
...
public:
    ...
    std::vector<r1cs_constraint<FieldT> > constraints_;

    r1cs_constraint_system() : primary_input_size_(0), auxiliary_input_size_(0) {}
...
#ifdef DEBUG
    std::map<size_t, std::string> constraint_annotations_;
    std::map<size_t, std::string> variable_annotations_;
#endif

    bool is_valid() const;
    bool is_satisfied(const r1cs_primary_input<FieldT> &primary_input,
                      const r1cs_auxiliary_input<FieldT> &auxiliary_input) const;
    bool is_satisfied(const r1cs_variable_assignment<FieldT> &full_variable_assignment) const;
