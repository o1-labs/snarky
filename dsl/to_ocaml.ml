...
let mk_lid name =
  Location.mkloc (Longident.Lident name.txt) name.loc
...
let rec of_expression = function
  | Apply (f, x) ->
    Exp.apply (of_expression f) [Nolabel, (of_expression x)]
  | Variable name ->
  ...
  | Fun (args, typ, body) ->
    let rec wrap_args args body =
      ...
      | (p, typ) :: args ->
        let pat =
          match typ with
          | Some typ ->
            Pat.constraint_ (of_pattern p) (of_typ typ)
          | None -> of_pattern p
        in
    ...
    let body =
      match typ with
      | Some typ ->
        Exp.constraint_ (of_expression body) (of_typ typ)
      | None -> of_expression body
    in
    wrap_args args body
  | Seq (e1, e2) ->
    Exp.sequence (of_expression e1) (of_expression e2)
  | Let (p, e_rhs, e) ->
    Exp.let_ Nonrecursive [Vb.mk (of_pattern p) (of_expression e_rhs)]
      (of_expression e)
