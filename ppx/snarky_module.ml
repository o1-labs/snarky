...
let rec unique_var_name ~modules var_name_map var_name' i =
  ...
  | None ->
      let var_name_map = Map.add_exn var_name_map ~key:var_name ~data:modules in
      (var_name_map, var_name)
...
let fields_pattern ~loc polyrecord =
  B.ppat_record ~loc
    (List.map polyrecord.fields ~f:(fun { label; _ } ->
         ({ txt = Lident label.txt; loc = label.loc }, B.ppat_var ~loc label) )
    )
    Closed
...
module Polydef = struct
  let parse_field var_name_map ({ loc; txt = name }, expr) =
    ...
    let var_name =
      match modules with
      | [ ModuleName { txt = ident; _ } ]
      | [ IdentName { txt = Ldot (ident, _); _ } ] ->
    ...
    let base_module, modules =
      match modules with
      | [ ( ModuleName { txt = ident; loc }
          | IdentName { txt = Ldot (ident, _); loc } )
        ] ->
          ({ txt = ident; loc }, [])
      | ( ModuleName { txt = ident1; loc }
        | IdentName { txt = Ldot (ident1, _); loc } )
        :: ( ModuleName { txt = ident2; _ }
           | IdentName { txt = Ldot (ident2, _); _ } )
           :: _ ->
  ...
  let build { name; fields } =
    let record_typ =
      Ptype_record
        (List.map fields ~f:(fun { label; var_name; _ } ->
             ...
             B.label_declaration ~loc ~name:label ~mutable_:Immutable ~type_ )
        )
    in
    let params =
    ...
    in
    let td =
      B.type_declaration ~loc:name.loc ~name ~params ~cstrs:[] ~kind:record_typ
    ...
    | PStr
        ...
        if Map.mem map name.txt then
          Location.raise_errorf ~loc:name.loc
            "A polymorphic record definition called %s already exists." name.txt ;
...
module Typedef = struct
  let build ~name ~map ~polyname ~current_module polyrecord =
    ...
    let td =
      B.type_declaration ~loc ~name ~params:[] ~cstrs:[] ~kind:Ptype_abstract
        ~private_:Public ~manifest:(Some bound_poly)
    in
...
module Snarkytyp = struct
  let build_fold ~loc ~name ~typ_mod ~fname polyrecord =
    let typ_fn f =
      B.pexp_ident ~loc:typ_mod.loc
        { txt = Ldot (typ_mod.txt, f); loc = typ_mod.loc }
    in
    List.fold_left
        ...
        B.pexp_apply ~loc (typ_fn "bind")
          ...
          ; ( Nolabel
            , B.pexp_fun ~loc Nolabel None
                (B.ppat_var ~loc:label.loc label)
                expr )
          ] )
                ...
                ( { pexp_desc =
                      Pexp_constraint
                        ...
                        , { ptyp_desc =
                              Ptyp_constr
                                ( typ
                                , [ { ptyp_desc = Ptyp_constr (polyname, [])
                                    ; _
                                    }
                                  ] )
                          ; _
  ...
  let build ~loc polyrecord =
    B.pstr_value ~loc Nonrecursive
      (List.map polyrecord.fields ~f:(fun { label; _ } ->
           ...
           let destr_record =
             B.ppat_record ~loc [ (label_ident, label_pat) ] Open
           in
...
let include_ ~loc ?(attr = []) mod_ =
  B.pstr_include ~loc
    { pincl_loc = loc; pincl_attributes = attr; pincl_mod = mod_ }

let snarky_module ~loc ~path:_ payload =
  match payload with
  | PStr structure ->
      let structure, _ =
        snarky_module_map#structure structure (Map.empty (module String), [], [])
      in
      include_ ~loc (B.pmod_structure ~loc structure)
  | _ ->
      Location.raise_errorf ~loc
        "Expected [%%snarky_module] to contain a structure."
