...
let rec check_type_aux ~loc typ ctyp env =
  ...
  *)
  ...
  | Trow row1, Trow row2 ->
      ...
      let row_closed =
        match (row_closed1, row_closed2) with Open, Open -> Open | _ -> Closed
      in
      let row_tags =
  ...
  | None -> (
      ...
      | Tctor variant1, Tctor variant2 -> (
          ...
          then
            ...
            | Some
                ({ tdec_desc = TRecord fields1; tdec_params = params1; _ }, None)
              -> (
                match
                  Envi.TypeDecl.unfold_alias_aux ~loc typ.type_alternate env
                with
                | Some
                    ( { tdec_desc = TRecord fields2; tdec_params = params2; _ }
                    , None ) ->
                    ...
                    let conv_field { fld_ident = ident1; fld_type = typ1 }
                      ...
                      in
                      let typ2 =
                        Envi.Type.instantiate params2 variant2.var_params typ2
                          env
                      in
                      if not (unifies env typ1.type_alternate typ2) then
                        ...
                        raise (Error (loc, env, Cannot_create_conversion typ)) ;
                      let free_vars', conv =
                        get_conversion_body !free_vars typ1
                      in
                      free_vars := free_vars' ;
...
let rec get_expression env expected exp =
  ...
  | Pexp_newtype (name, body) ->
      ...
      let decl, env =
        ...
        | Checked ->
            ...
            let tri_stitched = Type1.Mk.ctor ~mode:Prover 10000 other_name [] in
            let tri_stitched _ _ = tri_stitched in
            Typet.TypeDecl.import ~name ~tri_stitched ~newtype:true decl env
        | Prover ->
            (* Normal stitching. *)
            Typet.TypeDecl.import ~name ~other_name ~newtype:true decl env
      in
  ...
  | Pexp_seq (e1, e2) ->
      ...
      ( { exp_loc = loc; exp_type = e2.exp_type; exp_desc = Texp_seq (e1, e2) }
      , env )
  | Pexp_let (p, e1, e2) ->
      let env = Envi.open_expr_scope env in
  ...
  | Pexp_match (e, cases) ->
      ...
      ( { exp_loc = loc; exp_type = expected; exp_desc = Texp_match (e, cases) }
      , env )
  | Pexp_field (e, field) ->
      let field_info =
  ...
  | Pexp_record (((field, _) :: _ as fields), ext) ->
      ...
      ( { exp_loc = loc; exp_type = typ; exp_desc = Texp_record (fields, ext) }
      , !env )
  | Pexp_ctor (name, arg) ->
  ...
  | Pexp_if (e1, e2, Some e3) ->
      ...
      ( { exp_loc = loc
        ; exp_type = expected
        ; exp_desc = Texp_if (e1, e2, Some e3)
        }
      , env )
  | Pexp_prover e ->
...
let rec check_signature_item env item =
  ...
  | Psig_convtype (decl, tconv, convname) ->
      ...
      let decl, tconv, env = Typet.TypeDecl.import_convertible decl tconv env in
      let typ =
  ...
  | Psig_modtype (name, signature) ->
      ...
      ( env
      , { Typedast.sig_desc = Tsig_modtype (name, signature); sig_loc = loc } )
  | Psig_open name ->
  ...
  | Psig_request (arg, ctor_decl) ->
      ...
      let env, _variant, ctors =
        type_extension ~loc variant [ ctor_decl ] env
      in
      let ctor_decl, arg =
        match ctors with
        | [ ( { ctor_ret =
                  Some { type_desc = Ttyp_ctor { var_params = [ arg ]; _ }; _ }
              ; _
              } as ctor_decl )
          ] ->
            (ctor_decl, arg)
...
let rec check_statement env stmt =
  ...
  | Pstmt_type decl -> (
      ...
      try
        ...
        let name = Ident.create ~mode:Checked ~ocaml:true decl.tdec_ident.txt in
  ...
  | Pstmt_typeext (variant, ctors) ->
      let env, variant, ctors = type_extension ~loc variant ctors env in
      ( env
      , { Typedast.stmt_loc = loc; stmt_desc = Tstmt_typeext (variant, ctors) }
      )
  | Pstmt_request (arg, ctor_decl, handler) ->
      ...
      in
      ( env
      , { stmt_loc = loc; stmt_desc = Tstmt_request (arg, ctor_decl, handler) }
      )
...
let rec report_error ppf = function
  ...
  | Wrong_record_field (field, typ) ->
      fprintf ppf
        "@[<hov>This record expression is expected to have type@ @[<h>%a@]@;\
         The field %a does not belong to type@ @[<h>%a@].@]" !pp_typ typ
        Longident.pp field !pp_typ typ
  ...
  | Extension_different_arity lid ->
      fprintf ppf
        "@[<hov>This extension does not match the definition of type %a@;\
         They have different arities.@]" Longident.pp lid
  | Unmatched_cases cases ->
      fprintf ppf
        "This pattern-matching is not exhausive.@;\
         Here are some examples of cases that are not matched:@.%a"
  ...
  | Type_modes_mismatch (typ1, typ2) ->
      fprintf ppf
        "@[<hov>Internal error: the modes of these types do not match:@;\
         %a@;\
         %a@]"
        !pp_typ typ1 !pp_typ typ2
  | Missing_row_constructor (path, typ, constr_typ) ->
      fprintf ppf
        "@[<hov>Cannot unify@ @[<h>%a@] and@ @[<h>%a@]:@ the constructor %a is \
         not present in both.@]"
  ...
  | Int_out_of_range typ ->
      fprintf ppf
        "@[<hov>Integer literal exceeds the range of representable integers of \
         type@ %s.@]"
        typ
