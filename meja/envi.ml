...
module Scope = struct
  ...
  let register_type_declaration decl scope =
    ...
    | TAbstract | TAlias _ | TUnfold _ | TOpen ->
        scope
    | TRecord fields ->
  ...
  let rec find_module ~loc resolve_env lid scope =
    match lid with
    | Lident name ->
        get_module ~loc resolve_env name scope
    | Ldot (path, name) ->
        Option.bind
  ...
  and get_module ~loc resolve_env name scope =
    ...
    | Some (Immediate m) ->
        Some m
    | Some (Deferred lid) ->
        get_global_module ~loc resolve_env lid
    | None ->
        None
  ...
  and get_global_module ~loc resolve_env lid =
    ...
    match (m, lid) with
    | Some m, Some lid ->
        find_module ~loc resolve_env lid m
    | Some m, None ->
        Some m
    | None, _ ->
        None
...
let find_module ~loc (lid : lid) env =
  ...
  | None -> (
    ...
    | Some m ->
        m
    | None ->
        raise (Error (loc, Unbound_module lid.txt)) )
